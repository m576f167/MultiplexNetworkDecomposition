...
test_noise_result = mclapply(mc.cores = no_of_cores, seq(0, 0.1, 0.1/100),
			     function(noise_level, list_simulation, starting_point, epsilon){
				     return(TestNoise(list_simulation$network, list_simulation$traces_matrix, starting_point, rep(noise_level, length(list_simulation$network)), epsilon))
			     }, list_simulation_1$ER, starting_point, 1e-4)
```

```{r process_test_noise}
test_noise_max = lapply(test_noise_result,
			 function(result){
				 result[dim(result)[1], ]
			 })
```
